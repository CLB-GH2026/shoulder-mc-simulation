"""
Shoulder MC — ROTATOR-CUFF model, Combined 650 nm + 808 nm (Move+ device)
--------------------------------------------------------------------------
Device model with the Move+ strap in its as-worn over-the-shoulder geometry:
a vertical loop with modules supraclavicular (over supraspinatus), behind the
scapula (over infraspinatus), and posterior-inferior, closing under the arm.
This geometry deposits light in the peri-articular ROTATOR-CUFF / deltoid
musculature; the glenohumeral cartilage is bone-shadowed from these positions
(see the companion "Joint-Optimized" model for anterior/axillary joint access).
...
per-tissue fluence / absorption / illumination-zone coverage — watch the Muscle
group as the rotator-cuff dose metric.
...
def _default_src_configs(jl_z):
    """Move+ strap placement: the strap wraps obliquely over the shoulder (a
    vertical loop returning under the arm), NOT a horizontal ring at the joint
    line. The three co-located modules (both wavelengths) are stacked along Z in
    the sagittal plane (X~0) and aimed at the glenohumeral joint centre:
      1. Supraclavicular  — over the top of the shoulder, above the clavicle
                            (anterior + superior).
      2. Behind scapula   — posterior, roughly at joint height.
      3. Posterior-inferior — lower back of the shoulder, toward where the strap
                            returns under the arm (axilla)."""
    return [
        {'name': 'Supraclavicular',    'world_pos': [  0,  35, jl_z + 55], 'color': 'red'  },
        {'name': 'Behind scapula',     'world_pos': [  0, -65, jl_z +  5], 'color': 'green'},
        {'name': 'Posterior-inferior', 'world_pos': [  0, -50, jl_z - 55], 'color': 'blue' },
    ]
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = Path(f"results_shoulder_rotatorcuff_{RUN_ID}")
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    print("Shoulder MC — ROTATOR-CUFF model (over-shoulder strap), Combined 650 + 808 nm")
    ...
    for condition in MELANIN_808:
        ...
        if cond_results:
            results_to_csv(
                ...
                output_path=str(OUTPUT_DIR / f"MC_Shoulder_RotatorCuff_{condition}.csv"),
            )

    melanin_comparison_to_csv(
        all_condition_results, groups=COMP_GROUPS,
        output_path=str(OUTPUT_DIR / "MC_Shoulder_RotatorCuff_Melanin_Comparison.csv"),
