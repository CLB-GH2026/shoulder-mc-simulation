"""
Shoulder MC — Combined 650 nm + 808 nm (Move+ device model)
------------------------------------------------------------
The Move+ device runs its 650 nm and 808 nm emitters SIMULTANEOUSLY from
co-located pads. At PBM intensities the two wavelengths are physically
independent and MC photon transport is linear, so the tissue experiences the
SUM of the two fluence fields. This script models that directly: it builds the
subject geometry once, runs pmcx at each wavelength (its own optical properties
and source power, the same 3 co-located source positions), sums the fields, and
reports the COMBINED per-tissue fluence / absorption / illumination-zone
coverage — plus each wavelength's contribution.

This is the primary device-representative analysis. The separate
`SHO Models_MC Results_808nm.py` / `_650nm.py` scripts remain for
single-wavelength studies.
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = Path(f"results_shoulder_combined_{RUN_ID}")
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    print("Shoulder MC Simulation — Combined 650 + 808 nm (Move+ device)")
    ...
    for condition in MELANIN_808:
        ...
        if cond_results:
            results_to_csv(
                ...
                output_path=str(OUTPUT_DIR / f"MC_Shoulder_Combined_{condition}.csv"),
            )

    melanin_comparison_to_csv(
        all_condition_results, groups=COMP_GROUPS,
        output_path=str(OUTPUT_DIR / "MC_Shoulder_Combined_Melanin_Comparison.csv"),
