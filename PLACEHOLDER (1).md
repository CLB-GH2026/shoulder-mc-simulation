"""
Shoulder STL Mesh → 3D Voxel Volume + pmcx Fluence Overlay  (650 nm)
----------------------------------------------------------------------
...
def run_subject(subject_id, mesh_dir_base, output_dir, melanin_condition='fair'):
    ...
    tissues = {
        ...
        "labrum":        (mesh_dir / "labrum_raw.stl",                       5, opt(0.014,  2.00,  0.90, 1.37)),  # fibrocartilage
        "glenoid-cart":  (mesh_dir / "glenoid_cartilage_raw.stl",            8, opt(0.025,  1.20,  0.90, 1.37)),  # hyaline
        "humeral-cart":  (mesh_dir / "humeral_cartilage_raw.stl",            7, opt(0.025,  1.20,  0.90, 1.37)),  # hyaline
...
def _default_src_configs(jl_z):
    """
    Default source positions for the shoulder at 650 nm.
...
def results_to_csv(all_results, output_path="MC_Analysis_Shoulder_650nm.csv",
                   treatment_times_s=(300, 600, 900)):
    import csv
    GROUPS = {
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = Path(f"results_shoulder_650nm_{RUN_ID}")
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    print(f"Shoulder MC Simulation — 650 nm")
    ...
    for condition in MELANIN_CONDITIONS:
        ...
        if cond_results:
            results_to_csv(cond_results,
                           output_path=str(OUTPUT_DIR / f"MC_Shoulder_650nm_{condition}.csv"))

    melanin_comparison_to_csv(
        all_condition_results,
        output_path=str(OUTPUT_DIR / "MC_Shoulder_Melanin_Comparison_650nm.csv"),
        wavelength_nm=808,
    )
    print(f"\nDone.")
